...
const config = {
  ...
  lnbits: {
    ...
    webhookUrl: process.env.LNBITS_WEBHOOK_URL,
    webhookSecret: process.env.LNBITS_WEBHOOK_SECRET,
    environment: process.env.LNBITS_ENVIRONMENT || 'demo', // demo, production
    timeout: parseInt(process.env.LNBITS_TIMEOUT) || 30000, // 30 seconds
    retryAttempts: parseInt(process.env.LNBITS_RETRY_ATTEMPTS) || 3,
    retryDelay: parseInt(process.env.LNBITS_RETRY_DELAY) || 1000, // 1 second
  },
  conversionRates: {
