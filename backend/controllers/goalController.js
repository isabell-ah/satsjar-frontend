    ...
    if (userRole === 'child') {
      ...
      goalsSnapshot.forEach((doc) => {
        ...
        goals.push({
          ...
          deletionRequested: goalData.deletionRequested || false,
          deletionRequestedAt: goalData.deletionRequestedAt || null,
        });
      });
...
const updateGoal = async (req, res) => {
  try {
    ...
    // Handle deletion request fields
    if (deletionRequested !== undefined) {
      updateData.deletionRequested = deletionRequested;
    }

    if (deletionRequestedAt !== undefined) {
      updateData.deletionRequestedAt = deletionRequestedAt;
    }
    ...
    res.json({
      ...
      goal: {
        ...
        status: updatedGoalData.approved
          ...
          : 'pending',
        // Ensure these fields are included for the frontend
        target: updatedGoalData.targetAmount || 0,
        current: updatedGoalData.currentAmount || 0,
        approved: updatedGoalData.approved || false,
        deletionRequested: updatedGoalData.deletionRequested || false,
        deletionRequestedAt: updatedGoalData.deletionRequestedAt || null,
      },
    });
  } catch (error) {
...
const deleteGoal = async (req, res) => {
  try {
    ...
    // Check if goal has contributed funds that need to be refunded
    const contributedAmount = goalData.currentAmount || 0;

    if (contributedAmount > 0) {
      console.log(
        `Refunding ${contributedAmount} sats to child ${goalData.childId} for deleted goal "${goalData.name}"`
      );

      // Use transaction to ensure atomic operations
      await db.runTransaction(async (transaction) => {
        // Delete the goal
        transaction.delete(db.collection('goals').doc(goalId));

        // Refund contributed amount back to child's balance
        transaction.update(db.collection('children').doc(goalData.childId), {
          balance: admin.firestore.FieldValue.increment(contributedAmount),
        });
        ...
        transaction.set(transactionRef, {
          ...
          description: `Refund from deleted goal: ${goalData.name}`,
      ...
      res.json({
        ...
        refunded: contributedAmount,
        details: `${contributedAmount} sats refunded to your balance`,
      });
    } else {
      // No funds to refund, just delete the goal
  ...
  try {
    ...
    if (!goalData.approved) {
      return res.status(400).json({
        error: 'Cannot contribute to unapproved goal',
      });
    }

    // Check if goal is pending deletion
    if (goalData.deletionRequested) {
      return res.status(400).json({
        error: 'Cannot contribute to goal pending deletion',
      });
    }
