    ...
    plansSnapshot.forEach((doc) => {
      plans.push({
        ...
        nextExecution: doc.data().nextExecution
          ? doc.data().nextExecution.toDate().toISOString()
          : null,
        pausedAt: doc.data().pausedAt
          ? doc.data().pausedAt.toDate().toISOString()
          : null,
      });
    });
...
const toggleSavingsPlan = async (req, res) => {
  try {
    ...
    // If pausing the plan, set pausedAt timestamp
    // If activating the plan, remove pausedAt timestamp
    if (!newActiveStatus) {
    ...
    await db.collection('savings_plans').doc(planId).update(updateData);
...
const deleteSavingsPlan = async (req, res) => {
  try {
    ...
    if (userRole === 'child' && planData.childId !== userId) {
      return res.status(403).json({
        error: 'You do not have permission to delete this savings plan',
      });
    }
    ...
    await db.collection('savings_plans').doc(planId).delete();
